...
const DepartmentItem = ({ item, onClick }) => {
  ...
  return (
    <Row className="cursor-pointer mx-0 py-3" onClick={() => onClick && onClick(item)}>
      ...
      <Col xs={5} className="d-h-start pl-4 pr-0">
        <p className="dsl-b14 truncate-one text-500 mb-0">{}</p>
      </Col>
...
export const DepartmentFeed = ({ department, posts }) => {
      ...
      {departments.length > 0 ? (
        <>
          ...
          {departments.map((department, index) => (
            <DepartmentItem key={`department-${index}`} item={department} onClick={handleOpenDetail} />
          ))}
        </>
      ) : (
