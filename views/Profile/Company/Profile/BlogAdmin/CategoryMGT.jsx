  ...
  return (
    <Row className="mx-0 py-3">
      ...
      <Col xs={4} className="d-h-start">
        <p className="dsl-b14 text-500 truncate-one mb-0">{posts[0]?.name}</p>
      </Col>
...
const CategoryMGT = ({ blog, companyId, userId }) => {
  ...
  const handleSelectMenu = type => item => e => {
    switch (e) {
      case 'add category':
  ...
  const handleAddCategory = (e, item) => {
    ...
    const payload = {
      type: 'Add Category',
      data: {
        before: { type, data: item, blog, companyId, userId },
        after: { type: 'GETBUSINESS_REQUEST', payload: { id: companyId } },
      },
      callBack: {},
    }
    ...
    <div className="blog-admin-category-mgt">
      {categories.length > 0 ? (
        <>
          ...
          <CategoryMGTHeader />
          {categories.map(item => {
            return <CategoryMGTItem key={item.name} item={item} onSelect={handleSelectMenu('Public')} />
          })}
        </>
      ) : (
